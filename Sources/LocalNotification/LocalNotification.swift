...
/**
 * There are totally 3 kind of notification styles in iOS. They are: (User can toggle the available of different kinds of notification at the system Settings)
 ...
 * - Description: Local notifications are scheduled and “sent” locally on your iPhone, so they aren’t sent over the internet. Typical use cases are calendar reminders or the iPhone alarm clock.
 ...
 * - Remark: LocalNotification API is great for debugging when iOS is in suspended mode and logging doesn't work
 * - Remark: UserNotifications: lets us create notifications to the user that can be shown on the lock screen.
 * - Remark: We have two types of notifications to work with, and they differ depending on where they were created: local notifications are ones we schedule locally, and remote notifications (commonly called push notifications) are sent from a server somewhere.
...
public final class LocalNotification { // Used to debug restoration mode
    ...
    * - Remark: UNUserNotificationCenter handles all notification-related behavior in the app. This includes requesting authorization, scheduling delivery and handling actions
    * ## Examples
    ...
    * - Description: Let's us request a notification to be shown in a certain number of seconds from now
    * - Remark: The content is what should be shown, and can be a title, subtitle, sound, image, and so on.
    * - Remark: The trigger determines when the notification should be shown, and can be a number of seconds from now, a date and time in the future, or a location.
    * - Remark: The request combines the content and trigger, but also adds a unique identifier so you can edit or remove specific alerts later on. If you don’t want to edit or remove stuff, use UUID().uuidString to get a random identifier.
    * - Remark: You can also make date based triggers: `UNCalendarNotificationTrigger(dateMatching: calendar.current.dateComponents([.day, .month, .year, .hour, .minute], from: date)`
    * - Remark: Date example 20 secs from now: `let fireDate = Calendar.current.dateComponents([.day, .month, .year, .hour, .minute, .second], from: Date().addingTimeInterval(20))`
    * - Remark: There are also location based triggers
    * - Remark: It's also possible to add content to a notification via UNNotificationAttachment: see https://programmingwithswift.com/how-to-send-local-notification-with-swift-5/
    * - Remark: There is also badge in the content: update the application's badge number
    * - Remark: There is also content.userinfo like: content.userInfo = ["value": "Data with local notification"]  the information you want to associate with local notification. This is useful in cases, where we want to check the data associated with notification.
    * ## Examples:
    ...
    *   - subTitle: A secondary description for the notification. (As of iOS 10, you can define a title as well as a subtitle.)
    *   - timeInterval: Show this notification n seconds from now
    *   - id: Choose a random identifier (must be unique or it will only show the first, use "someID" + UUID().uuidString) 
...
extension LocalNotification {
    ...
    * - Remark: If the application was already granted permission to display notifications, we can go ahead and schedule a local notification.
    * - Remark: If no other switch cases match, i.e. in the case of .denied, we simply do nothing. We can’t ask for permission again, and we also can’t schedule local notifications
    * - Remark: On iOS, you can only ask for permission once. When permission has explicitly been denied, it has to be manually reset by the user via the iPhone’s Settings app. That’s why you always want to make it clear why an app is asking for permission. Many apps also use a 2-step approach, i.e. first ask for a soft permission (or prepare the user the permissions dialog is coming up), and then use the iOS-provided permission dialog.
   ...
   /**
    * - Remark: Why do we ask for the notification settings of the application? We do this to determine if the application previously requested the user's permission.
    */
